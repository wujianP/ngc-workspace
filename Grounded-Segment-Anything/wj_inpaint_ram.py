...
if __name__ == "__main__":
    ...
    # load RAM model
    ram_model = tag2text.ram(pretrained=args.ram_checkpoint,
                             image_size=384,
                             vit='swin_l').cuda()
    ram_model.eval()
    ...
    for iter_idx, (images, image_ids, Ws, Hs, objects) in enumerate(dataloader):
        ...
        tags_chinese = [tag.replace(' |', ',') for tag in ram_ret[1]]
        # empty cache
        ...
        # >>> Wandb visualize >>>
        wandb_visualize(images, tags_list, boxes_filt_list, masks_list, pred_phrases_list)
